...
struct HNSW {
  ...
  void add_links_starting_from(DistanceComputer &ptdis, storage_idx_t pt_id,
  ...
  int search_from_candidates(DistanceComputer &qdis, int k, idx_t *I, float *D,
                             MinimaxHeap &candidates, VisitedTable &vt,
                             int level, int nres_in = 0) const;

  std::priority_queue<Node>
  search_from_candidate_unbounded(const Node &node, DistanceComputer &qdis,
                                  int ef, VisitedTable *vt) const;

  /// search interface
  void search(DistanceComputer &qdis, int k, idx_t *I, float *D,
  ...
  static void shrink_neighbor_list(DistanceComputer &qdis,
