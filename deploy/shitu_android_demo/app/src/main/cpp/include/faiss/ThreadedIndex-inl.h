...
template <typename IndexT> ThreadedIndex<IndexT>::~ThreadedIndex() {
  for (auto &p : indices_) {
    if (isThreaded_) {
...
template <typename IndexT> void ThreadedIndex<IndexT>::addIndex(IndexT *index) {
  ...
  indices_.emplace_back(std::make_pair(
      index,
      std::unique_ptr<WorkerThread>(isThreaded_ ? new WorkerThread : nullptr)));
...
template <typename IndexT> void ThreadedIndex<IndexT>::reset() {
