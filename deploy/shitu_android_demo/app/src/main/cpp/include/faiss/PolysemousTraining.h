...
struct ReproduceDistancesObjective : PermutationObjective {
  ...
  ReproduceDistancesObjective(int n, const double *source_dis_in,
                              const double *target_dis_in,
                              double dis_weight_factor);

  static void compute_mean_stdev(const double *tab, size_t n2, double *mean_out,
                                 double *stddev_out);
...
struct PolysemousTraining : SimulatedAnnealingParameters {
  ...
  void optimize_pq_for_hamming(ProductQuantizer &pq, size_t n,
                               const float *x) const;
