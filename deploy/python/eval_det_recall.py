...
def cal_f1(gths, preds, thres=0.5):
    # tp, fp, tn, fn
    pr = [0, 0, 0, 0]
    for i in range(len(preds[0])):
        pr = [0, 0, 0, 0]
        for gth, pred in zip(gths, preds):
            match = []
            for g in gth:
                for j, p in enumerate(pred):
                    if j <= i:
                        if j not in match and cal_iou(p, g) >= thres:
                            pr[0] += 1
                            match.append(j)
                        else:
                            pr[1] += 1
            if match != len(gth):
                pr[3] += len(gth) - len(match)
        print(
            "det_iou_thres: {:.3f}, topk output: {}, pression: {:.4f}, recall: {:.4f}"
            .format(thres, i + 1, pr[0] / (pr[0] + pr[1]), pr[0] / (pr[0] + pr[
                3])))
...
def cal_iou(bbox1, bbox2):
    ...
    iou_w = max(min(bbox2[2], bbox1[2]) - max(bbox2[0], bbox1[0]), 0)
    iou_h = max(min(bbox2[3], bbox1[3]) - max(bbox2[1], bbox1[1]), 0)
    iou_area = iou_w * iou_h
    bbox1_area = (bbox1[3] - bbox1[1]) * (bbox1[2] - bbox1[0])
    bbox2_area = (bbox2[3] - bbox2[1]) * (bbox2[2] - bbox1[0])
    iou = iou_area / (bbox1_area + bbox2_area - iou_area)
    return iou


def main(config):
    det_predictor = DetPredictor(config)
    assert config["Global"]["infer_imgs"].endswith("json")
    ...
    for img_id in img_ids:
        ...
        img_file = os.path.join(img_root_path, img_anno["file_name"])

        ins_anno_ids = coco.getAnnIds(imgIds=[img_id], iscrowd=None)
        instances = coco.loadAnns(ins_anno_ids)
        gth.append([x["bbox"] for x in instances])
        img = cv2.imread(img_file)[:, :, ::-1]
        output = det_predictor.predict(img)
        pred.append([x["bbox"] for x in output])
    cal_f1(gth, pred)
