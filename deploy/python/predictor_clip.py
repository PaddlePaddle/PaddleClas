...
MODEL = ["image-to-text","image-to-image","image_index_build","text_index_build"]
...
class CLIPPredictor(Predictor):
    def __init__(self, config):
        ...
        if self.mode.find("text_index_build")!= -1:
        ...
        if self.mode.find("build") == -1:
            if self.mode.find("image-to-image") != -1:
...
if __name__ == "__main__":
    ...
    args.config = "deploy/configs/inference_CLIP.yaml"
