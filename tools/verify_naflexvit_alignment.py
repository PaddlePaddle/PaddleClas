...
device = sys.argv[6]
use_pretrained = sys.argv[7] == "1"

torch.manual_seed(2026)
torch.set_grad_enabled(False)
model = timm.create_model(variant, pretrained=use_pretrained).to(device)
model.eval()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify NaFlexViT forward alignment between timm and PaddleClas."
    )
    parser.add_argument(
        "--variant",
        default="naflexvit_base_patch16_gap",
        choices=[
    ...
    parser.add_argument(
        "--timm-root",
        default=str((Path(__file__).resolve().parents[2] / "pytorch-image-models")),
    )
    parser.add_argument(
        "--torch-python",
        default=str(
            (Path(__file__).resolve().parents[2] / "timm_env" / "bin" / "python")
        ),
    )
    parser.add_argument(
        "--paddle-python",
...
def main():
    ...
    x = (
        ...
        .standard_normal((args.batch_size, 3, args.height, args.width))
        .astype("float32")
    )
    ...
    with tempfile.TemporaryDirectory(prefix="naflexvit_align_") as tmpdir:
        ...
        feature_metrics = diff_metrics(
            torch_output["features"], paddle_output["features"]
        )
