...
def build(config, mode='train'):
    ...
    dataloader_config = config["DataLoader"][mode.capitalize()]

    batch_size = dataloader_config["sampler"]["batch_size"]

    file_root = dataloader_config["dataset"]["image_root"]
    file_list = dataloader_config["dataset"]["cls_label_path"]
    ...
    pad_output = False
    image_shape = config["Global"].get("image_shape", None)
    if image_shape and image_shape[0] == 4:
        pad_output = True

    transforms = {
        k: v
        for d in dataloader_config["dataset"]["transform_ops"]
        for k, v in d.items()
    }
