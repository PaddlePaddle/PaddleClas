...
class AdamWIBOT(optim.AdamW):
    """
    This optimizer can dynamically adjust the coefficient of weight_decay.
    """
    def __init__(self,
                 learning_rate=0.001,
                 beta1=0.9,
        ...
        if isinstance(weight_decay, str):
            weight_decay = [float(it) for it in weight_decay.split()]
            from .learning_rate import Cosine
            self.weight_decay = Cosine(
                self.learning_rate.epochs, self.learning_rate.step_each_epoch,
                self.learning_rate.learning_rate, self.learning_rate.eta_min
            )
        else:
            self.weight_decay = weight_decay
    ...
    def __call__(self, model_list):
        ...
        super().__init__(
            ...
            epsilon=self.epsilon,
            parameters=parameters,
            weight_decay=self.weight_decay if isinstance(self.weight_decay, float) else self.weight_decay.get_lr(),
            multi_precision=self.multi_precision,
            grad_clip=self.grad_clip,
            apply_decay_param_fun=self._apply_decay_param_fun)
    ...
    def wd_step(self):
        ...
        self.regularization = paddle.regularizer.L2Decay(self.weight_decay.get_lr())