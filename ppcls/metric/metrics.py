...
class MetricsPerClass(nn.Layer):
    ...
    def forward(self, x, label):
        ...
        for i in range(x.shape[0]):
            pre_label = int(paddle.argmax(x[i]))
            gt_label = int(label[i])
            # TP
            self.true_pos[gt_label].append(pre_label==label[i])
            # TN + FP
            self.gt_neg[gt_label] += 1
            # TP + FN
            self.gt_pos[pre_label] += 1
        return {}
    ...
    def avg_info(self):
        max_acc = 0.
        for key, value in self.true_pos.items():
            true_pos = paddle.to_tensor(value, dtype="float32").sum()
        ...
        result = f"Top1Acc: {max_acc}"
        self.acc_info = f"\n\tRecall\tPrecision" 
        for i in sorted(self.recall_dict):
            self.acc_info += f"\n{i}:\t" + str('%.4f' % self.recall_dict[i]) + "\t" + str('%.4f' % self.precision_dict[i])
