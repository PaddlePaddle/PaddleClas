...
class LinearTransformTeacher(nn.Layer):
    def __init__(self, qk_dim, t_shapes, keys):
        ...
        self.query_layer = nn.LayerList([LinearBNReLU(t_shape[1], qk_dim) for t_shape in self.t_shapes])
...
class LinearTransformStudent(nn.Layer):
    def __init__(self, qk_dim, t_shapes, s_shapes, keys):
        ...
        self.relu = nn.ReLU()
        self.samplers = nn.LayerList([Sample(t_shape) for t_shape in self.unique_t_shapes])
        self.key_layer = nn.LayerList([LinearBNReLU(s_shape[1], self.qk_dim) for s_shape in self.s_shapes])
