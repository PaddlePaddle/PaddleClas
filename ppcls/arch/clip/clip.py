...
class Attention(nn.Layer):
    def __init__(self,
        ...
        self.out_proj = nn.Linear(
            embed_dim, output_dim) if output_dim else nn.Linear(embed_dim,
                                                                embed_dim)
    ...
    def forward(self, x, attn_mask=None):
        ...
        q = q.reshape([L, N * self.num_heads, -1]).transpose(
            [1, 0, 2]) * self.scale
        k = k.reshape([L, N * self.num_heads, -1]).transpose(
            [1, 0, 2]) * self.scale
        v = v.reshape([L, N * self.num_heads, -1]).transpose(
            [1, 0, 2]) * self.scale

        if self.logit_scale is not None:
            attn = paddle.bmm(F.normalize(
                q, dim=-1),
                              F.normalize(
                                  k, axis=-1).transpose([0, 2, 1]))
    ...
    def text_global_pool(self, x, text=None, pool_type='first'):
        if pool_type == 'first':
            ...
            index = paddle.to_tensor(
                [paddle.arange(x.shape[0]), text.argmax(axis=-1)])
