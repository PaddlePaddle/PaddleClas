...
class PatchEmbed(nn.Layer):
    ...
    def _init_img_size(self, img_size):
        ...
        grid_size = (img_size[0] // self.patch_size[0],
...
def pixel_freq_bands(num_bands, max_freq=224., linear_bands=True, dtype='float32'):
    if linear_bands:
        bands = paddle.linspace(1.0, max_freq / 2, num_bands, dtype=dtype)
    else:
        bands = 2.0 ** paddle.linspace(0, math.log(max_freq, 2) - 1, num_bands, dtype=dtype)
    return bands * math.pi


def freq_bands(num_bands, temperature=10000., dtype='float32'):
    exp = paddle.arange(0, num_bands, dtype='int64').astype('float32') / num_bands
    return 1. / (temperature ** exp)


def build_fourier_pos_embed(feat_shape, bands=None, num_bands=64, max_res=224,
                            temperature=10000., linear_bands=False, include_grid=False,
                            in_pixels=True, ref_feat_shape=None,
                            grid_offset=0., grid_indexing='ij', dtype='float32'):
    if bands is None:
        if in_pixels:
    ...
    if grid_indexing == 'xy':
        ...
        if ref_feat_shape is not None:
            ref_feat_shape = [ref_feat_shape[1], ref_feat_shape[0]] + list(ref_feat_shape[2:])

    if in_pixels:
        t = [paddle.linspace(-1., 1., num=s, dtype='float32') for s in feat_shape]
    else:
        t = [paddle.arange(s, dtype='int64').astype('float32') + grid_offset for s in feat_shape]
...
def build_rotary_pos_embed(feat_shape, bands=None, dim=64, max_res=224,
                           temperature=10000., linear_bands=False, in_pixels=True,
                           ref_feat_shape=None, grid_offset=0.,
                           grid_indexing='ij', dtype='float32'):
    sin_emb, cos_emb = build_fourier_pos_embed(
        feat_shape, bands=bands, num_bands=dim // 4, max_res=max_res,
        temperature=temperature, linear_bands=linear_bands, in_pixels=in_pixels,
        ref_feat_shape=ref_feat_shape, grid_offset=grid_offset,
        grid_indexing=grid_indexing, dtype=dtype)
    num_spatial_dim = 1
    for s in feat_shape:
        num_spatial_dim *= s
    sin_emb = paddle.repeat_interleave(sin_emb.reshape([num_spatial_dim, -1]), repeats=2, axis=-1)
    cos_emb = paddle.repeat_interleave(cos_emb.reshape([num_spatial_dim, -1]), repeats=2, axis=-1)
...
class RotaryEmbeddingCat(nn.Layer):
    ...
    def __init__(self, dim, max_res=224, temperature=10000., in_pixels=True,
                 linear_bands=False, feat_shape=None, ref_feat_shape=None,
                 grid_offset=0., grid_indexing='ij'):
        super().__init__()
        self.dim = dim
        ...
        if self._use_cached_embed:
            ...
            self.register_buffer(
                'pos_embed', paddle.empty([num_pos, dim * 2], dtype='float32'),
                persistable=False)
            self.bands = None
        else:
            self.register_buffer(
    ...
    def _compute_bands(self):
        if self.in_pixels:
            return pixel_freq_bands(self.dim // 4, float(self.max_res),
    ...
    def _get_pos_embed_values(self, feat_shape, dtype='float32'):
        embeds = build_rotary_pos_embed(
            feat_shape=feat_shape, dim=self.dim, max_res=self.max_res,
            temperature=self.temperature, linear_bands=self.linear_bands,
            in_pixels=self.in_pixels, ref_feat_shape=self.ref_feat_shape,
            grid_offset=self.grid_offset, grid_indexing=self.grid_indexing, dtype=dtype)
        return paddle.concat(embeds, axis=-1)

    def get_embed(self, shape=None):
        if shape is not None and self.bands is not None:
            embeds = build_rotary_pos_embed(
                shape, bands=self.bands, in_pixels=self.in_pixels,
                ref_feat_shape=self.ref_feat_shape, grid_offset=self.grid_offset,
...
def global_pool_nlc(x, pool_type='avg', num_prefix_tokens=0):
    if not pool_type:
        return x
    if pool_type == 'token':
        return x[:, :num_prefix_tokens].mean(axis=1) if num_prefix_tokens > 1 else x[:, 0]
...
class GluMlp(nn.Layer):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None,
                 act_layer=nn.Silu, norm_layer=None, bias=True, drop=0.,
                 gate_last=True):
        super().__init__()
...
class EvaAttention(nn.Layer):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=True, qkv_fused=True,
                 qkv_bias_separate=False, num_prefix_tokens=1,
                 attn_drop=0., proj_drop=0., attn_head_dim=None,
                 norm_layer=None, qk_norm=False, scale_norm=True,
                 rotate_half=False):
        super().__init__()
        ...
        if qkv_fused:
            ...
            if qkv_bias:
                ...
                self.register_buffer(
                    'k_bias', paddle.zeros([attn_dim], dtype='float32'), persistable=False)
    ...
    def forward(self, x, rope=None, attn_mask=None):
        ...
        if self.qkv is not None:
            ...
            q = self.q_proj(x).reshape([B, N, self.num_heads, -1]).transpose([0, 2, 1, 3])
            k = self.k_proj(x).reshape([B, N, self.num_heads, -1]).transpose([0, 2, 1, 3])
            v = self.v_proj(x).reshape([B, N, self.num_heads, -1]).transpose([0, 2, 1, 3])
...
class EvaBlock(nn.Layer):
    def __init__(self, dim, num_heads, qkv_bias=True, qkv_fused=True,
                 ...
                 scale_mlp=False, scale_attn_inner=False,
                 num_prefix_tokens=1, attn_type='eva', rotate_half=False,
                 proj_drop=0., attn_drop=0., drop_path=0.,
                 init_values=None, act_layer=nn.GELU,
                 norm_layer=None, attn_head_dim=None, **kwargs):
        super().__init__()
        ...
        self.gamma_1 = self.create_parameter(
            shape=[dim], dtype='float32') if init_values is not None else None
        ...
        if scale_mlp or swiglu_align_to:
            ...
            self.mlp = GluMlp(
                in_features=dim, hidden_features=hidden_features * 2,
                norm_layer=norm_layer if scale_mlp else None,
                act_layer=nn.Silu, gate_last=False, drop=proj_drop)
...
class Eva(nn.Layer):
    ...
    def __init__(self,
                 ...
                 global_pool='avg', embed_dim=768, depth=12, num_heads=12,
                 qkv_bias=True, qkv_fused=True, mlp_ratio=4.,
                 swiglu_mlp=False, swiglu_align_to=0,
                 scale_mlp=False, scale_attn_inner=False,
                 attn_type='eva', drop_rate=0., pos_drop_rate=0.,
                 patch_drop_rate=0., proj_drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=None,
                 init_values=None, class_token=True, num_reg_tokens=0,
                 no_embed_class=False, use_abs_pos_emb=True,
                 use_rot_pos_emb=False, rope_type='cat',
                 rope_grid_offset=0., rope_grid_indexing='ij',
                 rope_temperature=10000., rope_rotate_half=False,
                 use_post_norm=False, use_pre_transformer_norm=False,
                 use_post_transformer_norm=None, use_fc_norm=None,
                 attn_pool_num_heads=None, attn_pool_mlp_ratio=None,
                 dynamic_img_size=False, dynamic_img_pad=False,
                 ref_feat_shape=None, head_init_scale=0.001):
        super().__init__()
        if norm_layer is None:
            norm_layer = partial(nn.LayerNorm, epsilon=1e-6)
        ...
        self.cls_token = self.create_parameter(
            shape=[1, 1, embed_dim], dtype='float32') if class_token else None
        self.reg_token = self.create_parameter(
            shape=[1, num_reg_tokens, embed_dim], dtype='float32') if num_reg_tokens else None

        num_pos_tokens = (num_patches if no_embed_class
                          else num_patches + self.num_prefix_tokens)
        self.pos_embed = self.create_parameter(
            shape=[1, num_pos_tokens, embed_dim],
            dtype='float32') if use_abs_pos_emb else None
        ...
        self.blocks = nn.LayerList([
            EvaBlock(
                ...
                attn_type=attn_type, rotate_half=rope_rotate_half,
...
def _compute_resize_matrix(old_size, new_size, interpolation='bicubic',
                           antialias=True, dtype='float32'):
    ...
    resized = F.interpolate(basis, size=[new_h, new_w], mode=interpolation,
                            align_corners=False)
    return resized.squeeze([1]).transpose([1, 2, 0]).reshape([new_total, old_total])


def resample_patch_embed(patch_embed, new_size, interpolation='bicubic',
    ...
    mat = _compute_resize_matrix(old_size, new_size, interpolation, antialias, 'float32')
...
def resample_abs_pos_embed(posemb, new_size, old_size=None, num_prefix_tokens=1,
    ...
    posemb = posemb.astype('float32').reshape([1, old_size[0], old_size[1], -1]).transpose([0, 3, 1, 2])
    posemb = F.interpolate(posemb, size=new_size, mode=interpolation, align_corners=False)
    posemb = posemb.transpose([0, 2, 3, 1]).reshape([1, -1, embed_dim]).astype(orig_dtype)
...
def checkpoint_filter_fn(state_dict, model, interpolation='bicubic', antialias=True):
    ...
    for k, v in state_dict.items():
        ...
        elif mim_weights and k in ('mask_token', 'lm_head.weight', 'lm_head.bias',
        ...
        if 'patch_embed.proj.weight' in k:
            ...
            if v.shape[-1] != W or v.shape[-2] != H:
                v = resample_patch_embed(v, (H, W),
                    interpolation=interpolation, antialias=antialias)
        elif k == 'pos_embed' and v.shape[1] != model.state_dict()['pos_embed'].shape[1]:
            npt = 0 if getattr(model, 'no_embed_class', False) else getattr(
                model, 'num_prefix_tokens', 1)
            v = resample_abs_pos_embed(v, new_size=model.patch_embed.grid_size,
                num_prefix_tokens=npt, interpolation=interpolation, antialias=antialias)
...
def EVA02_tiny_patch14_336(num_classes=1000, pretrained=None, **kwargs):
    model_args = dict(
...
def EVA02_small_patch14_336(num_classes=1000, pretrained=None, **kwargs):
    model_args = dict(
...
def EVA02_base_patch14_448(num_classes=1000, pretrained=None, **kwargs):
    model_args = dict(
...
def EVA02_large_patch14_448(num_classes=1000, pretrained=None, **kwargs):
    model_args = dict(
