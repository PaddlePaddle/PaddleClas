...
class Attention(TheseusLayer):
    ...
    def forward(self, x):
        ...
        attn = paddle.einsum("bhnd,bhmd->bhnm", q, k) * self.scale
        attn = F.softmax(attn, axis=-1)
        attn = self.attn_drop(attn)
        x = paddle.einsum("bhnm,bhmd->bhnd", attn, v)
        x = paddle.transpose(x, [0, 2, 1, 3]).reshape([b, n, c])
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class NaFlexEmbeds(TheseusLayer):
    def __init__(
        ...
        self.proj = nn.Linear(
            self.patch_size[0] * self.patch_size[1] * in_chans, embed_dim
        )
        self.reg_token = (
    ...
    def _apply_factorized_pos_embed(self, x, grid_size):
        ...
        pos = (pos_y.unsqueeze(2) + pos_x.unsqueeze(1)).reshape(
...
class NaFlexVit(TheseusLayer):
    def __init__(
        ...
        self.norm = (
            nn.LayerNorm(embed_dim, epsilon=norm_eps)
            if final_norm and not fc_norm
            else Identity()
        )
        self.fc_norm = (
            nn.LayerNorm(embed_dim, epsilon=norm_eps)
            if final_norm and fc_norm
            else Identity()
        )
...
def naflexvit_base_patch16_gap(pretrained=False, use_ssld=False, **kwargs):
    ...
    _load_pretrained(
        pretrained, model, MODEL_URLS["naflexvit_base_patch16_gap"], use_ssld
    )
    return model
...
def naflexvit_base_patch16_par_gap(pretrained=False, use_ssld=False, **kwargs):
    ...
    _load_pretrained(
        pretrained, model, MODEL_URLS["naflexvit_base_patch16_par_gap"], use_ssld
    )
    return model
