...
class PatchEmbed(nn.Layer):
    def __init__(self, patch_size, patch_stride, in_chans,
...
class FasterNet(nn.Layer):
    def __init__(self, in_chans=3, embed_dim=96, depths=(1, 2, 8, 2),
                 mlp_ratio=2.0, n_div=4, patch_size=4, patch_stride=4,
                 patch_size_t=2, patch_stride_t=2, patch_norm=True,
                 feature_dim=1280, drop_path_rate=0.1,
                 layer_scale_init_value=0, norm_layer='BN',
                 act_layer='RELU', class_num=1000, fork_feat=False,
                 init_cfg=None, pretrained=None, pconv_fw_type='split_cat',
                 **kwargs):
        super().__init__()
        if norm_layer == 'BN':
        ...
        dpr = [x.item() for x in paddle.linspace(start=0, stop=drop_path_rate,
                                                 num=sum(depths))]
        stages_list = []
        ...
        if self.fork_feat:
            ...
            self.head = (
                nn.Linear(in_features=feature_dim, out_features=class_num)
                if class_num > 0
                else nn.Identity()
            )
