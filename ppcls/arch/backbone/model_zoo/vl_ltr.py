...
from ....utils.save_load import load_dygraph_pretrain, load_dygraph_pretrain_from_url_state_dict
from .foundation_vit import MODEL_URLS,VisionTransformer,QuickGELU,Block
...
def interpolate_pos_embed(pos_embed_checkpoint: paddle.Tensor, new_patch_size, num_extra_tokens=1):
    ...
    pos_tokens = nn.functional.interpolate(pos_tokens, size=(new_patch_size, new_patch_size), mode='bicubic', align_corners=False)
...
class CVLP(nn.Layer):
    def __init__(self,
        ...
        self.proj = self.create_parameter(
            shape=(embed_dim,),
            default_initializer=Assign(
                embed_dim ** -0.5 * paddle.randn(((embed_dim, embed_dim)))
            )
        )
    ...
    def initialize_parameters(self, pretrained_clip):
        ...
        if pretrained_clip is not None:
            if self.model_type == "vit":
                pretrained_state_dict = load_dygraph_pretrain_from_url_state_dict(MODEL_URLS["CLIP_vit_base_patch16_224"])
            else:
            ...
            if isinstance(self.visual, VisionTransformer):
                num_extra_tokens = 1
                new_size = int((self.visual.pos_embed.shape[1] - num_extra_tokens) ** 0.5)
...
class LGR(nn.Layer):
    def __init__(self,
        ...
        self.visual = VisionTransformer(
                model_name = "CLIP",
                img_size=image_resolution,
                patch_size=vision_patch_size,depth=vision_layers,
                num_heads=vision_heads, embed_dim=embed_dim
            )
        ...
        if op_type is None:
            ...
            self.text_block = TextBlock(dim=embed_dim, num_heads=attn_heads,
                                    qkv_bias=False, qk_scale=None, drop=0,
                                    attn_drop=0,op_type=op_type,
    ...
    def load_pretrained_model(self, vis_backbone_path=None, img_grad=True,
        ...
        split_text_embeddings = paddle.transpose(split_text_embeddings,(0,1,2))
    ...
    def _load_vis_backbone(self, vis_backbone_path):
        ...
        if isinstance(self.visual, VisionTransformer):
            num_extra_tokens = 1
            new_size = int((self.visual.pos_embed.shape[1] - num_extra_tokens) ** 0.5)
            new_pos_embed = interpolate_pos_embed(pretrained_state_dict['visual.pos_embed'], 
                                                    new_size, num_extra_tokens=num_extra_tokens)
