...
class AntiAliasDownsampleLayer(nn.Layer):
    def __init__(self, remove_aa_jit: bool = False, filt_size: int = 3, stride: int = 2,
        ...
        # self.op = BlurPool2d(channels, filt_size, stride)
...
#             nn.BatchNorm2D(nf, momentum=0.1, use_global_stats=True),
...
class TResNet(nn.Layer):
    def __init__(self, layers, in_chans=3, class_num=1000, width_factor=1.0, remove_model_jit=False, **kwargs):
        ...
        # conv1 = conv2d(in_chans * 16, self.planes, stride=1)
        ...
        for m in self.sublayers():
            if isinstance(m, BasicBlock):
                # m.conv2[1].weight.set_value(paddle.zeros_like(m.conv2[1].weight))
            ...
            if isinstance(m, Bottleneck):
                # m.conv3[1].weight.set_value(paddle.zeros_like(m.conv3[1].weight))
                if isinstance(m, Conv2d_ABN):
                    if isinstance(m, nn.BatchNorm2D):
                        Zeors = nn.initializer.Constant(0.0)
                        Zeors(m.weight)
...
if __name__=="__main__":
    model = TResNetM()
    model.set_state_dict(paddle.load("./paddle.pdparams"))
    model.eval()
    import numpy as np
    np.random.seed(123)
    input_np = np.random.rand(1, 3, 224, 224).astype(np.float32)
    input_pd = paddle.to_tensor(input_np)
    output_pd = model(input_pd)
    output_np = output_pd.numpy()
    print(output_np[0][:5])
