...
class VisionTransformer(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, class_num=1000, embed_dim=768, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, lin_probe=False, sin_pos_emb=True, args=None):
        super().__init__()
    ...
    def reset_classifier(self, class_num, global_pool=''):
        self.class_num = class_num
        self.head = nn.Linear(self.embed_dim, class_num) if class_num > 0 else nn.Identity()
...
def _load_pretrained(pretrained, model, model_keys, model_ema_configs, abs_pos_emb, rel_pos_bias, use_ssld=False):
...
def cae_base_patch16_224(pretrained=pretrained_model, use_ssld=False, **kwargs):
    ...
    model = VisionTransformer(
        ...
        norm_layer=partial(nn.LayerNorm, epsilon=1e-6), **config)
