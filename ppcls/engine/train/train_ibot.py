...
def train_epoch_ibot(engine, epoch_id, print_batch_step, wd_schedule, momentum_schedule, freeze_last_layer):
    tic = time.time()
    for iter_id, (images, labels,masks) in enumerate(engine.train_dataloader):
        ...
        for i, param_group in enumerate(engine.optimizer._param_groups):
            # engine.optimizer.set_lr(engine.lr_schedule[it])
            if i == 0:
        ...
        if engine.amp:
            ...
            with paddle.amp.auto_cast(level=amp_level):
                ...
                loss = engine.train_loss_func(student_out, teacher_out,student_local_cls, masks, epoch_id)
        else:
            student_out, teacher_out, student_local_cls = engine.model.forward(images,masks)
            loss = engine.train_loss_func(student_out, teacher_out,student_local_cls, masks, epoch_id)
