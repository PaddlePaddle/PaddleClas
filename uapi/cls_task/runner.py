...
class ClsRunner(BaseRunner):
    def train(self, config_file_path, cli_args, device):
        ...
        cmd = f"{python} tools/train.py -c {config_file_path} {cli_args}"
    ...
    def predict(self, config_file_path, cli_args, device):
        ...
        cli_args = " ".join(cli_args)
        cmd = f"{self.python} tools/infer.py -c {config_file_path} {cli_args}"
        self.run_cmd(cmd, switch_wdir=True, echo=True, silent=False)

    def export(self, config_file_path, cli_args, device):
        # `device` unused
        cli_args = " ".join(cli_args)
        cmd = f"{self.python} tools/export_model.py -c {config_file_path}  {cli_args}"
        self.run_cmd(cmd, switch_wdir=True, echo=True, silent=False)
    ...
    def compression(self, config_file_path, cli_args, device, save_dir, model_name):
        ...
        self.export(config_file_path, cli_args, device)