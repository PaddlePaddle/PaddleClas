...
def load_label_name_dict(path):
    ...
    for line in open(path, 'r'):
        ...
        try:
            ...
            raise Exception(
                'Please loading legal format as "classid classname"')
    return result
...
def parse_args(mMain=True, add_help=True):
    ...
    if mMain == True:
        ...
        # params for predict
        parser.add_argument(
            "--model_file", type=str)  ## 生成模型结构文件（inference.pdmodel）
        parser.add_argument(
            "--params_file", type=str)  ## 模型权重文件（inference.pdiparams）
...
class PaddleClas(object):
            ...
            if process_params.model_name in model_names:
                ...
                if not os.path.exists(
                        os.path.join(download_path,
                                     process_params.model_name)):
                    os.makedirs(
                        os.path.join(download_path, process_params.model_name))
                download_path = os.path.join(download_path,
                                             process_params.model_name)
                maybe_download(model_storage_directory=download_path, url=url)
                process_params.model_file = download_path + '/inference.pdmodel'
                process_params.params_file = download_path + '/inference.pdiparams'
    ...
    def predict(self, img):
        ...
        Returns:
            dict：{image_name: "", class_id: [], scores: [], label_names: []}，if label name path == None，label_names will be null.
        """
