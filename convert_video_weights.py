...
def convert_video_model(model_name: str, checkpoint: str, output_dir: str = None, cache_dir: str = None):
...
def main():
    parser = argparse.ArgumentParser(description="Convert Hiera video model PyTorch weights to Paddle format")
    parser.add_argument("--model", type=str, default=None, 
                        help="Model name to convert (default: all)")
    parser.add_argument("--checkpoint", type=str, default="mae_k400_ft_k400",
                        help="Checkpoint type (default: mae_k400_ft_k400)")
    ...
    if args.list:
        print("Available video models and checkpoints:")
        print("=" * 60)
    ...
    if args.model:
        convert_video_model(args.model, args.checkpoint, args.output_dir, args.cache_dir)
    else:
        print("Converting all video models...")
        print("=" * 60)
        for model_name in MODEL_URLS.keys():
            print(f"\n{'=' * 60}")
            print(f"Converting: {model_name}")
            print("=" * 60)
            convert_video_model(model_name, args.checkpoint, args.output_dir, args.cache_dir)
