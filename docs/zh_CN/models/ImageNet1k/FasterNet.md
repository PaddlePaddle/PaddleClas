...
为了实现更快的网络，团队重新审视了流行的运算符，并证明这种低的FLOPS主要是由于运算符频繁的内存访问，特别是深度卷积。因此，团队提出了一种新的部分卷积（PConv），通过同时减少冗余计算和内存访问来更有效地提取空间特征。在PConv的基础上，进一步提出了FasterNet，这是一个新的神经网络家族，它在各种设备上实现了比其他设备更高的运行速度，而不会影响各种视觉任务的准确性。例如，在ImageNet-1k上，小小的FasterNet-T0是2.8×，3.3×，还有2.4倍分别比GPU、CPU和ARM处理器上的MobileViT-XXS更快，同时2.9%更准确。大型FasterNet-L取得了令人印象深刻的成就83.5%顶级1的准确性，与新兴的Swin-B相当，同时拥有36%GPU上的推断吞吐量更高，以及节省37%在CPU上计算时间。
