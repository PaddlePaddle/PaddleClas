...
为了实现更快的网络，团队重新审视了流行的运算符，并证明低浮点运算次数（FLOPS）的主要原因是运算符频繁的内存访问，尤其是在深度卷积中。为此，团队提出了一种新的部分卷积（PConv）方法，通过同时减少冗余计算和内存访问，从而更有效地提取空间特征。在PConv的基础上，进一步推出了FasterNet，这是一个新的神经网络家族，能够在各种设备上实现比其他模型更高的运行速度，而不影响视觉任务的准确性。例如，FasterNet-T0在ImageNet-1K上分别比MobileViT-XXS在GPU、CPU和ARM处理器上快2.8倍、3.3倍和2.4倍，同时提高了2.9%的准确率。FasterNet-L也取得了显著成就，达到了83.5%的顶级1准确性，与新兴的Swin-B相当，同时在GPU上的推断吞吐量提高了36%，并节省了37%的CPU计算时间。
