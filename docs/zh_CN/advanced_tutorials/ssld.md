...
实战部分提供了 SSLD 蒸馏的示例，在 [PPLCNet_x2_5_ssld.yaml](../../../ppcls/configs/ImageNet/Distillation/PPLCNet_x2_5_ssld.yaml) 中提供了 `ResNet50_vd` 蒸馏 `PLCNet_x2_5` 的配置文件，用户可以在 `tools/train.sh` 里直接替换配置文件的路径即可使用。
...
当用户配置完训练环境后，配置下面的命令即可完成SSLD知识蒸馏的训练过程。
...
更多关于知识蒸馏的算法介绍以及模型评估、预测、导出等内容，请参考：[知识蒸馏实战教程](./knowledge_distillation.md)。
...
* 若用户准备添加无标签的训练数据，只需要将新的训练数据放置在原本训练数据的路径下，生成新的数据 list 即可，另外，新生成的数据 list 需要将无标签的数据添加伪标签（只是为了统一读取数据）。
