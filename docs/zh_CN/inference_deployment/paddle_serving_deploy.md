...
- [4.图像识别服务部署](#4图像识别服务部署)
- [4.1 模型转换](#41-模型转换)
- [4.2 服务部署和请求](#42-服务部署和请求)
    - [4.2.1 Python Serving](#421-python-serving)
    - [4.2.2 C++ Serving](#422-c-serving)
- [5.FAQ](#5faq)
...
- 关闭服务：  
需要特别说明的是：在图像识别部署过程中，每当检索库更新时，均需要重启服务才能应用最新的检索库。  
可以在启动服务路径或者环境变量SERVING_HOME路径下(该路径下存在文件ProcessInfo.json)执行如下关闭服务命令，并重新执行启动服务命令：  
```
